...
var randomGenerator = function (m) {
  m = m || 9
  var s = ''
...
$(document).ready(function () {
  var i
  var talkID = randomGenerator(10)
  console.log(talkID)

  $('#Talk').attr('id', talkID)
  $('#' + talkID).attr('style', 'display: block !important')
  ...
  $stage.dialog.profileDress.on('click', function (e) {
    ...
    if (showDialog($stage.dialog.dress)) {
...
const BETWEEN_CHAT_MINIMUM_MILLS = 50
const MINIMUM_CHAR_COUNT_PER_CHAT = 8
  ...
  var fun = function (msg) {
    ...
    switch (msg.type) {
      case 'keydown':
        ...
        if (currentTime - lastChatMap.keyTime <= BETWEEN_CHAT_MINIMUM_MILLS) {
          ...
          reportCheat(BETWEEN_CHAT_MINIMUM_MILLS + 'ms 내에 연속적으로 입력하였습니다.', false)
        }
      ...
      case 'chat':
        ...
        if (chatText.length - lastChatMap.lastChat.length >= MINIMUM_CHAR_COUNT_PER_CHAT) {
          if (chatText.slice(lastChatMap.lastChat.length).split('').some(function (v, i, a) {
            return a.lastIndexOf(v) !== i
          })) return // 같은 글자가 반복되는 경우 예외
          reportCheat('한번에 ' + MINIMUM_CHAR_COUNT_PER_CHAT + '글자 이상을 입력하였습니다.', true)
        }

        /*
        TODO: 자음감지 오작동이 심해 비활성화
        if (chatText.length - lastChatMap.lastChat.length === 1) {
          var lastChar = chatText.slice(-1)
          if (!/^[가-힣]$/.test(lastChar)) break
          if (lastChar.toKorChars()[0] === lastChatMap.lastChat.slice(-1).toKorChars()[2]) break
          // 새 글자의 초성과 전 글자의 종성이 같음

          reportCheat('초성 입력 없이 한글을 입력하였습니다.', true)
        }
        TODO: 키입력 감지 오작동이 심해 비활성화
      ...
      case 'keyup':
        var keycode = msg.value
        if (lastChatMap.lastKey !== keycode) {
          reportCheat('keyown과 keyup가 같지 않습니다.', false)
        }
        break
    }
  }
  ...
  switch (data.type) {
    ...
    case 'welcome':
      ...
      Object.defineProperty($data, '_testt', {
        value: addInterval(function () {
          if ($stage.talk.val() != $data._ttv) {
            cheatDetection({type: 'chat', value: $stage.talk.val()}, true)
            if (tailuserEnabled) send('cheatreport', {ev: 'c', v: $stage.talk.val()}, true)
      ...
      Object.defineProperties(document, {
        'onkeydown': {
          value: function (e) {
            cheatDetection({type: 'keydown', value: e.keyCode}, true)
            if (tailuserEnabled) send('test', {ev: 'd', c: e.keyCode}, true)
          }
        },
        'onkeyup': {
          value: function (e) {
            cheatDetection({type: 'keyup', value: e.keyCode}, true)
...
function replayTick (stay) {
  ...
  replayStatus()
  if ($data.room.events.length > $data._rf) {
    $data._rt = addTimeout(replayTick,
