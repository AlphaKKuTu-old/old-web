...
$lib.Classic.roundReady = function (data) {
  var len = $data.room.game.title.length
  var $l
...
function checkAge () {
  ...
  while (true) {
    ...
    if (lv === 4) {
      confirm(L['checkAgeSure'] + '\n' +
      input[0] + L['YEAR'] + ' ' +
      input[1] + L['MONTH'] + ' ' +
      input[2] + L['DATE'])
        .then((test) => {
          if (test) return send('caj', { answer: 'yes', input: [ input[1], input[2], input[0] ] }, true)
        })
    } else {
      confirm(L['checkAgeCancel'])
        .then((test) => {
          if (test) return send('caj', { answer: 'no' }, true)
        })
    }
  }
}
...
function checkRoom (modify) {
  ...
  var OBJ = {} + ''
  var i
  var arr = $data._players.split(',')
  var lb = arr.length
  var la = $data.room.players.length
  var u
...
function prettyTime (time) {
  ...
  var sec = Math.floor(time * 0.001) % 60
...
function drawScore ($obj, score) {
  var i
  var sc = (score > 99999) ? (zeroPadding(Math.round(score * 0.001), 4) + 'k') : zeroPadding(score, 5)
...
function getLevel (score) {
  ...
  for (var i = 0; i < l; i++) if (score < EXP[i]) break
  return i + 1
}
...
function stopAllSounds () {
  for (var i in $_sound) $_sound[i].stop()
}
function tryJoin (id) {
  var pw
